...
//   ],
//   imageSrc = "/services_image/mern-showcase.webp",
...
const ServiceShowcase = ({
  ...
  return (
    ...
    >
      ...
      {/* Glow blobs — radial-gradient only, NO blur filter */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-40 -bottom-52 h-[520px] w-[520px] rounded-full"
        style={{
          background:
            "radial-gradient(circle, rgba(255,101,51,0.25) 0%, rgba(255,101,51,0.1) 40%, transparent 70%)",
          transform: "translateZ(0)",
        }}
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-44 -top-40 h-[520px] w-[520px] rounded-full"
        style={{
          background:
            "radial-gradient(circle, rgba(255,101,51,0.22) 0%, rgba(255,101,51,0.08) 40%, transparent 70%)",
          transform: "translateZ(0)",
        }}
      />
      ...
      <div className="container relative z-10 mx-auto grid items-center gap-10 px-5 sm:px-8 lg:grid-cols-[1.05fr_1fr] lg:gap-14 xl:gap-16">
        {/* Image */}
        <div className="flex justify-center">
          <div className="overflow-hidden rounded-xl shadow-[0_25px_50px_rgba(0,0,0,0.18)] ring-1 ring-black/5 max-w-xl w-full bg-white/60">
            <img
              src={imageSrc}
              alt={imageAlt}
              className="h-full w-full object-cover"
        ...
        <div className="flex flex-col gap-5 text-[#2f2f2f]">
          ...
          <p className="text-sm md:text-base lg:text-lg xl:text-xl text-[#616161]">
            {description}
          </p>
          ...
          <ul className="space-y-2 text-sm md:text-base lg:text-lg text-[#616161]">
            {bullets.map((item, idx) => (
              <li key={idx} className="flex gap-2">
                <span
                  className="mt-2.5 inline-block h-1.5 w-1.5 rounded-full bg-orange-bg-cta shrink-0"
                  aria-hidden="true"
                />
                <span>{item}</span>
              </li>
