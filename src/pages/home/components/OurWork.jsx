...
const OurWork = () => {
  ...
  useGSAP(() => {
    cardsRef.current.forEach((card, index) => {
      // Pin each card at 8% from top; no extra spacing so cards stack on top of each other
      ScrollTrigger.create({
        trigger: card,
        start: "top 8%",
        endTrigger: containerRef.current,
      ...
      // Rotate the incoming card from right to center (skip first card)
      if (index > 0) {
        gsap.fromTo(card,
          {
            rotation: 10,
      ...
      // Rotate previous card to the left when this card scrolls up
      if (index > 0) {
        gsap.to(cardsRef.current[index - 1], {
          rotation: -5,
          scale: 0.96,
  ...
  return (
    // KEY FIX: Removed `overflow-x-clip` from the section.
    // `overflow: clip` creates a new containing block that clips
    // GSAP's `position: fixed` pinned elements, making cards invisible.
    // The decorative blur is now self-contained so it won't cause
    // horizontal scroll on its own.
    <section className="w-full text-white py-4 xl:pt-20 2xl:pt-24 relative">

      {/* Decorative blur wrapped in its own overflow-hidden container
          so it doesn't bleed horizontally without affecting pinned children */}
